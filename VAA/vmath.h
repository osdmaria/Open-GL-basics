// Module vmath.h tiré des exemples du "Red Book" :
//   https://github.com/openglredbook/examples/blob/master/include/vmath.h
//
// BUGFIX 09/01/2025 - Edouard.Thiel@univ-amu.fr
//   fonction ortho() : erreur de signe corrigée
    ...
    static inline mat4 ortho(float left, float right, float bottom, float top, float n, float f)
    {
        return mat4(vec4(2.0f / (right - left), 0.0f, 0.0f, 0.0f),
                    ...
                    vec4((left + right) / (left - right),
                         ...
                         // (n + f) / (f - n), 1.0f)
                         (n + f) / (n - f), 1.0f) // BUGFIX: erreur de signe
        );
    }
